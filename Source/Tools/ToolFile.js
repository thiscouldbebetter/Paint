...
class ToolFile extends Tool {
    ...
    processSaveAsTAR(event) {
        ...
        for (var i = 0; i < layers.length; i++) {
            ...
            var layerAsBytes = Base64Encoder.base64StringToBytes(layerAsBase64String);
            var layerAsTarFileEntry = TarFileEntry.fileNew(layer.name + ".png", layerAsBytes);
    ...
    controlUpdate() {
        if (this.control == null) {
            var containerSave = new ControlContainer("containerSave", [
                new ControlLabel("Save As:"),
                new ControlTextBox("textFileName", // id
                "Untitled", this.processFileNameToSaveAsChange.bind(this)),
                new ControlButton("As PNG", this.processSaveAsPNG.bind(this)),
                new ControlButton("As TAR", this.processSaveAsTAR.bind(this)),
            ]);
            var containerLoad = new ControlContainer("containerLoad", [
                new ControlLabel("Load:"),
                new ControlFileUploader("fileToLoad", this.fileToUploadChanged.bind(this)),
            ]);
            var returnValue = new ControlContainer("containerToolFile", [
                containerSave,
                containerLoad,
            ]);
    ...
    fileToUploadChanged(fileName, fileType, fileContentAsBytes) {
        if (fileContentAsBytes != null) {
            if (fileType == "image/png") {
                ...
                var tarFile = TarFile.fromNameAndBytes(fileName, fileContentAsBytes);
    ...
    layerAddFromPNGAsBytes(fileContentAsBytes) {
        ...
        imageLoadedAsImgElement.onload = (event) => {
            var imageSize = new Coords(imageLoadedAsImgElement.width, imageLoadedAsImgElement.height);
            ...
            layerNew.display.drawOther(imageLoadedAsDisplay, new Coords(0, 0), null, null // ?
            );
            view.controlUpdate();
        };
        var fileContentAsBase64 = Base64Encoder.bytesToBase64String(fileContentAsBytes);
